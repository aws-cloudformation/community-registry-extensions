...
import com.awscommunity.kms.encryptionsettings.model.aws.rds.dbcluster.AwsRdsDbcluster;
import com.awscommunity.kms.encryptionsettings.model.aws.rds.dbcluster.AwsRdsDbclusterTargetModel;
...
@EqualsAndHashCode
@ToString
public final class AwsRdsDbClusterKmsSettingsValidationImpl implements AwsKmsIntegratedService {
     ...
     */
    @Override
    public AwsRdsDbcluster getResourcePropertiesFromHookContext() {
        final HookContext hookContext = request.getHookContext();
        final AwsRdsDbclusterTargetModel awsRdsDbclusterTargetModel = hookContext
                .getTargetModel(AwsRdsDbclusterTargetModel.class);
        final AwsRdsDbcluster awsRdsDbcluster = awsRdsDbclusterTargetModel.getResourceProperties();
    ...
    public ProgressEvent<HookTargetModel, CallbackContext> validateAwsKmsSettings() {
        final AwsRdsDbcluster awsRdsDbcluster = getResourcePropertiesFromHookContext();

        final String kmsKeyId = awsRdsDbcluster.getKmsKeyId();
        if (kmsKeyId != null) {
        ...
        final String snapshotIdentifier = awsRdsDbcluster.getSnapshotIdentifier();
        final String sourceDBClusterIdentifier = awsRdsDbcluster.getSourceDBClusterIdentifier();
        final Boolean storageEncrypted = awsRdsDbcluster.getStorageEncrypted();
        if (isNullOrEmpty(snapshotIdentifier) && isNullOrEmpty(sourceDBClusterIdentifier)) {
